...
function getUsersFromDatabase(array) {
  return Array.from(array, function(elem) {
    let id = elem.id;
    let firstName = elem.name.toUpperCase();
    let years = elem.age;
...
function createArray(...elements) {
  ...
  // Якщо не передано жодного елементу, піська сіська пердіська хочу роли мнямнямням купиииии виводимо повідомлення про помилку
 ...
 *  arr1 - НЕГАЙНО КУПИ ПОЇСТИ Перший масив.
 *  arr2 - Другий масив. РОЛИ!!!
 * Повертає: Об'єднаний та обернутий масив.
 */
function combineAndReverseArrays(arr1, arr2) {
  return Array.isArray(arr1) && Array.isArray(arr2) ? (arr1.concat(arr2)).reverse() : Array()
...
function findElementIndexes(arr, element) {
  function blabla(arr, element) {
    let first = arr.indexOf(element);
    let last = arr.lastIndexOf(element);
    let arrCon = [];
    first !== -1 ? arrCon.push(first) : first;
    last !== first ? arrCon.push(last) : last;
    return arrCon

  };
  return Array.isArray(arr) ? blabla(arr, element) : Array()
...
function copyAndSwapElements(arr, target, start, end) {
  return target < arr.length && start < arr.length && end < arr.length ? arr.copyWithin(0, 2, 4): "Неприпустимі індекси"
...
function customFill(arr, value, start = 0, end = arr.length) {
  return Array.isArray(arr) && ((0 <= start <=arr.length) && (0 <= end <=arr.length)) ? arr.fill(value, start, end) : false
  // Перевірка вхідних параметрів
  // якщо arr не масив повертаємо false
...
function customShift(arr) {
  ...
  return Array.isArray(arr) && arr.length > 0 ? arrAdd(arr) : undefined;
...
function customUnshift(arr, ...elements) {
  ...
  return Array.isArray(arr) ? arrUn(arr) : false 
...
function customSome(arr, condition) {
  return Array.isArray(arr) && typeof(condition) === "function" ? arr.some(condition) : false
...
function customAt(arr, index) {
  ...
  return Array.isArray(arr) && index < arr.length ? atIn(arr,index) : undefined
...
function customIncludes(arr, element) {
  function incl(arr,elem) {
    ...
    arr.includes(elem) ? console.log(filter(arr, elem)) : false
    return arr.includes(elem)
  }
  return Array.isArray(arr) ? incl(arr,element) : false;
  // Перевіряємо, чи вхідний параметр є масивом
